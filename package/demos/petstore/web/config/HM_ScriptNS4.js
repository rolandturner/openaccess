...
function HM_f_MakeTrees(){
	...
	if(HM_StatusDisplayBuild) datalTrees + " Hierarchical Menu Trees Created";
    HM_AreCreated = true;
    HM_BeingCreated = false;
}
...
function HM_f_MakeMenu(menucount) {
	...
	while (HM_CurrentMenu.itemCount < HM_CurrentMenu.maxItems) {
		...
		if(!HM_CurrentItem) {
			...
			if(HM_StatusDisplayBuild) datang Hierarchical Menus: " + menucount + " / " + HM_CurrentMenu.itemCount;
...
function HM_f_MenuOut() {
    this.isOn = false;
    HM_UserOverMenu = false;
    if(HM_StatusDisplayLink) data if(!HM_ClickKill) HM_HideAllTimer = setTimeout("HM_CurrentMenu.hideTree()",10);
}
...
function HM_f_ItemOver(){
    ...
    if(HM_StatusDisplayLink) datankText;
