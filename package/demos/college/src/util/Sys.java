...
public class Sys {
    ...
    public static PersistenceManager pm() {
        ...
        if (pm == null || pm.isClosed()) {
            ...
            if (DEBUG) state.eventId = Versant.getMostRecentEventID();
        }
        return pm;
    }
    ...
    /**
     * Get all the SQL executed since the last call as HTML. Note that this
     * will return SQL executed by all Threads. This returns "" if DEBUG is
     * false.
     */
    public static String getNewSQL() {
        if (DEBUG) return Versant.getNewSQL(getState().eventId);
        return "";
    }
