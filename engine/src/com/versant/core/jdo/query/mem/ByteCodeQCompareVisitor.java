...
public class ByteCodeQCompareVisitor implements MemVisitor {
    ...
    public ByteCodeQCompareVisitor(ClassGen classGen, InstructionFactory factory, String name, ClassMetaData candidateClass) {
        ...
        mg = new MethodGen(Constants.ACC_PUBLIC, // access flags
                                        ...
                                        new String[] {"state1", "state2"}, // arg names
        ...
        LocalVariableGen var1 = mg.addLocalVariable("compResult",
                Type.INT, null, null);
    ...
    private Field visitStateFieldNavNodeRoot(FieldNavNode node, ClassMetaData currentClass) {
        ...
        switch(f.category){
            case FieldMetaData.CATEGORY_REF:
                ...
                il.append(new ALOAD(stateParamNo));//load the state
    ...
    private Field visitFieldNodeImp(FieldMetaData fmd, FieldNode node) {
        ...
        if (first) {
            first = false;
            il.append(new ALOAD(stateParamNo));//load state on stack
            ...
            if (fmd.type.isPrimitive()) {
                switch (fmd.typeCode) {
                    ...
                    case MDStatics.FLOAT:
                        il.append(factory.createInvoke(Float.class.getName(), "compare", Type.INT,
                            new Type[] {Type.FLOAT, Type.FLOAT}, Constants.INVOKEVIRTUAL));
                        break;
                    case MDStatics.DOUBLE:
                        il.append(factory.createInvoke(Double.class.getName(), "compare", Type.INT,
                            new Type[] {Type.DOUBLE, Type.DOUBLE}, Constants.INVOKEVIRTUAL));
