...
public final class VersantOid implements Externalizable  {
    ...
    public VersantOid(PCStateMan sm, ModelMetaData jmd, boolean resolved) {
        ...
        if (sm.oid.isNew() && ((NewObjectOID)sm.oid).realOID != null) {
            //don't set the sm if the actual oid is available.
    ...
    public VersantOid(String s) {
        try {
        	...
        	{
            	throw BindingSupportImpl.getInstance().invalidObjectId("Invalid OID String: '" + s + "': " + e,
                    e);
            }
        }
    }
    ...
    private void checkClassIDDigit(int digit, char c, String s) {
        if (digit < 0 || digit > 9) {
            throw BindingSupportImpl.getInstance().invalidObjectId("Invalid digit '" + c +
                    "' in classID for OID String: '" + s + "'", null);
        }
    }
    ...
    private void checkPkDigit(long digit, char c, String s) {
        if (digit < 0 || digit > 9) {
            throw BindingSupportImpl.getInstance().invalidObjectId("Invalid digit '" + c +
                    "' in primary key for OID String: '" + s + "'", null);
        }
    }
        ...
        if (newOid.realOID != null) {
            ...
            if (ce != null) {
                ...
                if (sm != null && !sm.getPmProxy().isClosed()) {
                    ...
                    throw BindingSupportImpl.getInstance().exception("The transaction is which this " +
                            "JDO Object Id was used was never finished");
                }
            } else {
                throw BindingSupportImpl.getInstance().exception("The transaction is which this " +
                        "JDO Object Id was used was never finished");
