...
public class DetachSCOFactoryRegistry {
     ...
     */
    public VersantSCOCollectionFactory getJDOGenieSCOCollectionFactory(
            FieldMetaData fmd) {
        if (fmd.category == MDStatics.CATEGORY_COLLECTION) {
            switch (fmd.typeCode) {
                ...
                default:
                    throw BindingSupportImpl.getInstance().notImplemented("Creating a SCO instance for field " +
                            fmd.getName() + " of type " + MDStaticUtils.toSimpleName(fmd.typeCode) +
                            " is not supported");
            }
        }
        return null;
    ...
    public VersantSCOMapFactory getJDOGenieSCOMapFactory(FieldMetaData fmd) {
        if (fmd.category == MDStatics.CATEGORY_MAP) {
            switch (fmd.typeCode) {
                ...
                default:
                    throw BindingSupportImpl.getInstance().internal("Type code not supported as map. TypeCode = " +
