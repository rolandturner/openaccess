...
public class DetachSCOTreeSetFactory implements VersantSCOCollectionFactory,
        Serializable {

    /**
     * Create a new Detached SCOTreeSet .
     * <p/>
    ...
    public VersantSimpleSCO createSCOCollection(PersistenceCapable owner,
        ...
        return new DetachSCOTreeSet(owner, stateManager, fmd, Utils.getObjectsById(collectionData.values,
                collectionData.valueCount, pm, (FieldMetaData) fmd,
                fmd.isElementTypePC()));
    }

    /**
     * Create a new SCOTreeSet instance and fill it with the data in collection.
     */
    public VersantSimpleSCO createSCOCollection(PersistenceCapable owner,
                                                 VersantPMInternal pm, VersantStateManager stateManager,
                                                 VersantFieldMetaData fmd, Collection collection) {
        return new DetachSCOTreeSet(owner, stateManager, fmd, collection.toArray());
    }
