...
public class ConfigParser {
    ...
    // This is no longer used - the Workbench will automatically convert
    // it to ext.jdbc-class-id instead on load
    public static final String JDBC_INHERITANCE_NO_CLASSID =
            "jdbc-inheritance-no-classid";
    ...
    public static final String VERSANT_IGNORE_CON_FACT_PROPS = "versant.ignore.con.factory.props";
    public static final String VERSANT_DATASOURCE_ENLISTED = "versant.datasource.enlisted";
    ...
    public ConfigInfo parse(Properties p) {
        ...
        c.allowPmCloseWithOpenTx = getBoolean(p, ALLOW_PM_CLOSE_WITH_OPEN_TX,
                DEFAULT_ALLOW_PM_CLOSE_WITH_OPEN_TX);
        c.precompileNamedQueries = getBoolean(p, PRECOMPILE_NAMED_QUERIES,
                DEFAULT_PRECOMPILE_NAMED_QUERIES);
        c.checkModelConsistencyOnCommit = getBoolean(p,
                CHECK_MODEL_CONSISTENCY_ON_COMMIT,
                DEFAULT_CHECK_MODEL_CONSISTENCY_ON_COMMIT);
        c.interceptDfgFieldAccess = getBoolean(p, INTERCEPT_DFG_FIELD_ACCESS,
                false);
        ...
        if (s.equals(PM_CACHE_REF_TYPE_SOFT)) {
            ...
            throw BindingSupportImpl.getInstance().runtime("Invalid " + PM_CACHE_REF_TYPE +
                    ": '" + s + "', expected " + PM_CACHE_REF_TYPE_SOFT + ", " +
                    PM_CACHE_REF_TYPE_WEAK + " or " +
                    PM_CACHE_REF_TYPE_STRONG);
        }

        c.retainValues = getBoolean(p, OPTION_RETAINVALUES,
                DEFAULT_OPTION_RETAINVALUES);
        c.restoreValues = getBoolean(p, OPTION_RESTORE_VALUES,
                DEFAULT_OPTION_RESTORE_VALUES);
        c.optimistic = getBoolean(p, OPTION_OPTIMISTIC,
                DEFAULT_OPTION_OPTIMISTIC);
        c.nontransactionalRead = getBoolean(p, OPTION_NON_TRANSACTIONAL_READ,
                DEFAULT_OPTION_NON_TRANSACTIONAL_READ);
        c.nontransactionalWrite = getBoolean(p, OPTION_NON_TRANSACTIONAL_WRITE,
                DEFAULT_OPTION_NON_TRANSACTIONAL_WRITE);
        c.ignoreCache = getBoolean(p, OPTION_IGNORE_CACHE,
                DEFAULT_OPTION_IGNORE_CACHE);
        c.multithreaded = getBoolean(p, OPTION_MULTITHREADED,
                DEFAULT_OPTION_MULTITHREADED);
        c.connectionFactoryName = p.getProperty(OPTION_CONNECTION_FACTORY_NAME);
        c.connectionFactory2Name = p.getProperty(
                OPTION_CONNECTION_FACTORY2_NAME);
        c.enlistedConnections = getBoolean(p, VERSANT_DATASOURCE_ENLISTED, false);

        // datastore tx locking
        ...
        if (s == null) {
            ...
            if (s.equals(DATASTORE_TX_LOCKING_ALL)) {
                ...
                throw BindingSupportImpl.getInstance().runtime("Invalid datastore.tx.locking: '" +
                        s + "', expected none, first or all.");
            }
        }
        ...
        c.remotePmpoolEnabled = getBoolean(p, REMOTE_PMPOOL_ENABLED,
                DEFAULT_REMOTE_PMPOOL_ENABLED);
        c.remotePmpoolMaxIdle = getInt(p, REMOTE_PMPOOL_MAX_IDLE,
                DEFAULT_REMOTE_PMPOOL_MAX_IDLE);
        c.remotePmpoolMaxActive = getInt(p, REMOTE_PMPOOL_MAX_ACTIVE,
                DEFAULT_REMOTE_PMPOOL_MAX_ACTIVE);

        c.queryCacheEnabled = getBoolean(p, QUERY_CACHE_ENABLED,
                DEFAULT_QUERY_CACHE_ENABLED);
        c.maxQueriesToCache = getInt(p, QUERY_CACHE_MAX_QUERIES,
                DEFAULT_CACHE_MAX_QUERIES);
        ...
        c.cacheListenerClass = getClassAndProps(p, CACHE_CLUSTER_TRANSPORT,
                c.cacheListenerProps = new HashMap());
        if (c.cacheListenerClass == null && c.cacheListenerProps == null) {
            c.cacheListenerClass = getClassAndProps(p, CACHE_LISTENER,
                    c.cacheListenerProps = new HashMap());
        }
        ...
        c.logDownloaderClass = getClassAndProps(p, LOG_DOWNLOADER,
                c.logDownloaderProps = new HashMap());

        c.metricStoreCapacity = getInt(p, METRIC_STORE_CAPACITY,
                DEFAULT_METRIC_STORE_CAPACITY);
        c.metricSnapshotIntervalMs = getInt(p, METRIC_SNAPSHOT_INTERVAL_MS,
                DEFAULT_METRIC_SNAPSHOT_INTERVAL_MS);

        // parse user defined base metrics
    ...
    private void readExternalizers(Properties p, ConfigInfo c) {
        ...
        for (int i = 0; i < MAX_EXTERNALIZER_COUNT; i++) {
            ...
            if (s != null) { // we have an externalizer
    ...
    public static int getExtEnum(Properties p, String be, int ext,
            Map map, int def) {
    ...
    public static boolean getExtBoolean(Properties p, String be, int ext,
            boolean def) {
        JdoExtension e = getExt(p, be, ext);
        if (e == null) return def;
        return e.getBoolean();
