...
//  }
    ...
    protected void initialize(Object patternObj, int cflags, RESyntax syntax, int myIndex, int nextSub) throws REException {
        ...
        while (index < pLength) {
            ...
            //  what is proper interpretation of '{' at start of string?
                ...
                if (newIndex > index) {
                    if (minMax.first > minMax.second)
                        throw new REException("interval minimum is greater than maximum",REException.REG_BADRPT,newIndex);
            ...
            //  [...] | [^...]
                ...
                if ((ch = pattern[index]) == '^') {
                    negative = true;
                    if (++index == pLength) throw new REException("expected end of character class",REException.REG_EBRACK,index);
                    ch = pattern[index];
                }
            ...
            //  (...) | \(...\) depending on RE_NO_BK_PARENS
                ...
                while ( ((nextIndex = getCharUnit(pattern,endIndex,unit)) > 0)
                    ...
                    if ((endIndex = nextIndex) >= pLength)
                        throw new REException("expected end of subexpression",REException.REG_ESUBREG,nextIndex);
                    else if (unit.ch == '(' && (syntax.get(RESyntax.RE_NO_BK_PARENS) ^ unit.bk))
                        nested++;
                    else if (unit.ch == ')' && (syntax.get(RESyntax.RE_NO_BK_PARENS) ^ unit.bk))
                        nested--;
    ...
    private int getMinMax(char[] input,int index,IntPair minMax,RESyntax syntax) throws REException {
        ...
        if (buf.length() == 0) {
            if (mustMatch)
                throw new REException("interval is empty or contains illegal chracters",REException.REG_EBRACE,index);
            else
                return startIndex;
        }
