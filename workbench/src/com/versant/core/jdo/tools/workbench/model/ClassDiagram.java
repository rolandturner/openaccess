...
public class ClassDiagram {
    ...
    public ClassDiagram(MdProject project) {
     ...
     * Copy constructor.
     */
    public ClassDiagram(ClassDiagram s) {
        project = s.project;
    ...
    public ClassInfo removeClass(MdClass mdClass) {
        ClassInfo ans = (ClassInfo)infoMap.remove(mdClass.getQName());
        makeDirty();
        return ans;
    }
     ...
     */
    public void save(PropertySaver p, String base){
        finishLoad();
        p.add(base + ConfigParser.DIAGRAM_NAME, settings.getName());
        StringList list = new StringList();
        list.append(legendX);
        list.append(legendY);
        p.add(base + ConfigParser.DIAGRAM_LEGEND, list.toString());
    ...
    public void load(Properties p, String base) {
        ...
        StringListParser ps = new StringListParser(
                p.getProperty(base + ConfigParser.DIAGRAM_LEGEND, "0,0"));
        legendX = ps.nextInt();
        legendY = ps.nextInt();
    ...
    public void finishLoad() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            ci.load(parser);
            infoMap.put(ci.getMdClass().getQName(), ci);
        }
        linesFromLoad = null;
    }
    ...
    public ClassInfo findClassInfo(MdClass mdClass) {
        return (ClassInfo)infoMap.get(mdClass.getQName());
    }
    ...
    public Set getClasses() {
        ...
        for (Iterator i = infoMap.keySet().iterator(); i.hasNext();) {
            ...
            if (o instanceof String){
                MdClass mdc = project.findClass((String)o);
                if (mdc != null) {
                    a.add(mdc);
                }
            }
    ...
    public abstract class Info implements Comparable {
        ...
        /**
         * Return our cell or null if none.
         */
    ...
    public static class Settings {
        ...
        public void save(Properties p) throws IOException {
            p.setProperty(KEY_GENERAL, encodeGeneral());
            p.setProperty(KEY_CLASS, encodeClass());
            p.setProperty(KEY_FIELD, encodeField());
        }

        public void load(Properties p) {
            decodeGeneral(p.getProperty(KEY_GENERAL));
            decodeClass(p.getProperty(KEY_CLASS));
            decodeField(p.getProperty(KEY_FIELD));
        }
         ...
         */
        public int countFieldFlags(boolean isJDBC) {
            int t = 0;
            if (fieldName) ++t;
            if (fieldType) ++t;
            if (fieldColumnName && isJDBC) ++t;
            if (fieldSQLType && isJDBC) ++t;
            return t;
        }
