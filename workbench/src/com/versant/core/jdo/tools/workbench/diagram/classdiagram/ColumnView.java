...
/**
 * View for a GraphColumn.
 *
 * @keep-all
 */
public class ColumnView extends VertexView implements CellViewRenderer {
    ...
    public ColumnView(Object o, JGraph jGraph, CellMapper cellMapper) {
    ...
    public void refresh(boolean createDependentViews) {
        ...
        if (!(parent instanceof TableView)) return;
        FieldListColModel cm = ((TableView)parent).getColModel();
    ...
    public Component getRendererComponent(JGraph graph, CellView view,
        ...
        GraphColumn c = getGraphColumn();
        flc.setForeground(color);
        populateFieldList(s, c, flc);
        flc.getColModel().reCalcColWidths();
        return flc;
    }

    private GraphColumn getGraphColumn() {
        ColumnCell cell = (ColumnCell)getCell();
        return cell.getGraphColumn();
    }

    private void populateFieldList(ClassDiagram.Settings s, GraphColumn c,
            FieldListComponent flc) {
        flc.clear();
        String name = c.getColumnName();
        if (name == null) name = "{auto}";
        flc.addColumn(name, 0);
        flc.addColumn(c.getSqlDDL(), 0);
