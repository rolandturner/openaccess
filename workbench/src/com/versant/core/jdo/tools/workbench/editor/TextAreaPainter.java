...
//        setAntiAliasing(gfx);
                                  ...
                                  Color defaultColor, int x, int y)
    {
        ...
        while (tokens.id != Token.END) {
            offsetShift = backWidth * getRealLength(textLine.substring(charOffset, charOffset + tokens.length),
                    4);
    ...
    protected void paintCaret(Graphics gfx, int line, int y) {
        int position = textArea.getBracketPosition();
        if (position != -1) {
            int offset = textArea.getCaretPosition() - textArea.getLineStartOffset(line) - 1;
            int caretX = textArea.offsetToX(line, offset);
            gfx.setColor(bracketHighlightColor);
            // Hack!!! Since there is no fast way to get the character
            // from the bracket matching routine, we use ( since all
            // brackets probably have the same width anyway
            gfx.fillRect(caretX, (y+ fm.getLeading() + fm.getMaxDescent()), fm.charWidth('(') - 1, fm.getHeight() - 1);
        }
        if (textArea.isCaretVisible()) {
            ...
            if (textArea.isOverwriteEnabled()) {
                ...
                gfx.drawLine(caretX+1, y, caretX+1, y + height - 1);
            }
        }
    }
