 ...
 * @keep-all
 */
public class InsightBody extends FormPanel {
    ...
    protected InsightBody() throws Exception {
        super("main");
    ...
    public void setListData(Object[] data) {
        ...
        model.setColumnWidth(0,FieldDisplay.BIGGEST_LENGHT+8);
        if (data.length < 12) {
     ...
     */
    public String getHeader() { return lblHeader.getText(); }
     ...
     */
    public int getSelectedIndex() { return grid.getSelectedIndex(); }
     ...
     */
    public int getListSize() { return grid.getModel().getRowCount(); }

    /**
     * Get the selected value in the list.
     */
    public Object getSelectedValue() { return grid.getSelectedObject(); }
    ...
    public void setInsightWord(String word) {
        for (int i = 0; i < grid.getModel().getRowCount(); i++) {
            ...
            if (word.regionMatches(false, 0, str, 0, word.length())) {
                setSelectedIndex(i);
                break;
            }
        }
