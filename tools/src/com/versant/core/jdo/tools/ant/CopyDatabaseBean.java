...
public class CopyDatabaseBean {
    ...
    private void prepare() {
        ...
        if (url != null) {
            destProps.setProperty(ConfigParser.STD_CON_URL,
                    url);
        }
        if (user != null) {
        ...
        if (properties != null) {
            ...
            destProps.setProperty(ConfigParser.STORE_PROPERTIES,
                    properties);
        }
    ...
    public void copyDatabase() throws Exception {
        ...
        try {
            ...
            out.println("=== Copying main table for each class (one dot = " +
                    rowsPerTransaction + " rows) ===\n");
            long mainStart = System.currentTimeMillis();
            int mainTotRows = 0;
            int numClasses = srcClasses.length;
            for (int i = 0; i < numClasses; i++) {
                mainTotRows += copyClassMainTable(srcClasses[i],
                        destClasses[i]);
                checkStopFlag();
            }
            ...
            out.println("=== Copying link tables for each class (one dot = " +
                    rowsPerTransaction + " rows) ===\n");
            ...
            for (int i = 0; i < numClasses; i++) {
                ...
                for (int j = 0; j < sc.fields.length; j++) {
                    ...
                    if (sfmd.storeField instanceof JdbcLinkCollectionField) {
                        JdbcLinkCollectionField srcField =
                                (JdbcLinkCollectionField)sfmd.storeField;
                        if (!srcField.readOnly) {
                            linkTotRows += copyLinkTable(srcField,
                                    (JdbcLinkCollectionField)destClasses[i].fields[j].storeField);
                            checkStopFlag();
                        }
                    }
                }
            ...
            out.println("Copied " + mainTotRows + "  main table rows " +
                    "in " + fmtSec(mainTime) + " secs (" +
                    perSec(mainTotRows, mainTime) + " per second)");

            out.println("Copied " + linkTotRows + "  link table rows " +
                    "in " + fmtSec(linkTime) + " secs (" +
                    perSec(linkTotRows, linkTime) + " per second)");
    ...
    private int copyClassMainTable(ClassMetaData srcClass,
                                   ClassMetaData destClass) throws Exception {
        ...
        try {
            ...
            enableIdentityInsert(autoinc, destClass, true);
            PersistGraph pg = new PersistGraph(dest.getJmd(),
                    rowsPerTransaction);

            QueryDetails qp = new QueryDetails();
            qp.setCandidateClass(srcClass.cls);
            qp.setSubClasses(false);
            qp.setFetchGroupIndex(
                    srcClass.getFetchGroup(FetchGroup.ALL_COLS_NAME).index);
            ...
            int tot = 0;
            int dots = 0;
            src.getSm().begin(false);
            rs = (JdbcQueryResult)src.getSm().executeQuery(
                    DummyApplicationContext.INSTANCE, null, cq,
                    null).getRunningQuery();
            ...
            for (; ;) {
                ...
                QueryResultContainer qResultContainer = new QueryResultContainer(container);
                rs.nextBatch(DummyApplicationContext.INSTANCE, 0, qResultContainer);
                Object[] oids = qResultContainer.getDataArray();

                int n = oids == null ? 0 : oids.length;
                more = !qResultContainer.isqFinished();
                for (int i = 0; i < n; i++, c++) {
                    JdbcGenericOID oid = (JdbcGenericOID) oids[i];
                    if (oid == null) break;
                    JdbcGenericState state = (JdbcGenericState) container.get(oid);
            ...
            enableIdentityInsert(autoinc, destClass, false);
    ...
    private void enableIdentityInsert(boolean autoinc, ClassMetaData destClass, boolean value) throws SQLException {
        Connection destCon = dest.pool.getConnection(false, false);
        if (autoinc) {
            dest.sqlDriver.enableIdentityInsert(destCon,
                    ((JdbcClass)destClass.storeClass).table.name, value);
        }
        if (destCon != null) destCon.close();
    }
    ...
    private int copyLinkTable(JdbcLinkCollectionField srcField,
                              JdbcLinkCollectionField destField) throws Exception {
        ...
        try {
            ...
            destCon = dest.getSm().getJdbcConnectionSource().getConnection(false, false);
            ...
            rs = stat.executeQuery(
                    srcField.getFetchAllRowsSql(src.getSm()));
            if (src.getSm().getSqlDriver().isFetchSizeSupported()) {
                rs.setFetchSize(rowsPerTransaction);
            }

            JdbcLinkCollectionField.LinkRow row =
                    new JdbcLinkCollectionField.LinkRow();
            ...
            if (destCon != null) dest.getSm().getJdbcConnectionSource().returnConnection(destCon);
        }
    }
    ...
    private class MiniServer {
        ...
        public MiniServer(Properties props) throws Exception {
            ...
            factory = getJdbcStoragemanagerFactory(b.createStorageManagerFactory());
        ...
        private JdbcStorageManagerFactory getJdbcStoragemanagerFactory(StorageManagerFactory smf) {
            while(true) {
                if (smf.getInnerStorageManagerFactory() == null) {
                    return (JdbcStorageManagerFactory) smf;
                } else {
                    smf = smf.getInnerStorageManagerFactory();
                }
            }
        }
        ...
        public void dropAllTables() throws Exception {
            ...
            try {
                ...
                for (int i = 0; i < a.size(); i++) {
                    ...
                    String name = (String)dbTableNames.get(
                            t.name.toLowerCase());
                    if (name != null) {
        ...
        public void createAllConstraints() throws Exception {
            out.println(
                    "=== Adding constraints in: " + pool.getURL() + " ===");
            Connection con = null;
            Statement stat = null;
            try {
                con = getAutoCommitCon();
    ...
    private static class DummyKeyGen implements JdbcKeyGenerator {
        ...
        public void generatePrimaryKeyPost(String className,
                                           JdbcTable classTable, Object[] data, Connection con,
