...
public final class JDBCConnectionPool
    ...
    private final String urlNoPassword;
    ...
    public JDBCConnectionPool(JdbcConfig jdbcConfig, LogEventStore pes,
        ...
        urlNoPassword = Utils.removePassword(jdbcConfig.url);
        ...
        userProps.put("url", jdbcConfig.url);
    ...
    public void init() {
        ...
        cleanupThread = new Thread(this, "VOA Pool " + urlNoPassword);
    ...
    private Connection createRealCon() {
        ...
        if (pes.isFine()) {
            ev = new JdbcConnectionEvent(0, null, urlNoPassword,
                    JdbcConnectionEvent.CON_OPEN);
            pes.log(ev);
        }
        Connection realCon = null;
        try {
            realCon = jdbcDriver.connect(userProps.getProperty("url"), props);
            if (realCon == null) {
                throw BindingSupportImpl.getInstance().fatalDatastore(
                        formatConnectionErr("Unable to connect to " + urlNoPassword));
            }
    ...
    public String toString() {
        PoolStatus ps = new PoolStatus(urlNoPassword);
        fillStatus(ps);
        return ps.toString();
    }
