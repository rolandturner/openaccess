...
/**
 * Buffer holding the SQL for a query. This supports modification of the
...
public class SqlBuffer {
    ...
    public SqlBuffer getClone() {
        SqlBuffer clone = new SqlBuffer();
    ...
    public void setParamList(Param paramList) {
        if (this.paramList != null) {
            throw BindingSupportImpl.getInstance().internal("Over writing the current param list");
        }
    ...
    public void createSpace(CharBuf charBuf, int index, int amount) {
        sql = null;
        for (SqlBuffer.Param p = paramList; p != null; p = p.next) {
     ...
     * @param toExcl Index of row after last row to return
     */
    public synchronized String getSql(SqlDriver driver, Object[] params,
            boolean forUpdate, boolean forCount, long fromIncl, long toExcl) {
        boolean changed = sql == null;
        if (params != null) {
            int paramIndex = 0;
            for (SqlBuffer.Param p = paramList; p != null; p = p.next, paramIndex++) {
                if (params[p.declaredParamIndex] instanceof Collection) {
                    ...
                    if (p.inListParamCount == 0) {
                        ...
                        for (int i = 0; i < n; i++) {
                            ...
                            System.arraycopy(charsToInsert, 0, chars, offset, charsToInsert.length);
    ...
    public void setParamsOnPS(ModelMetaData jmd, SqlDriver driver,
        ...
        int pos = 1;
        SqlBuffer.Param p = paramList;
        if (p == null && params != null && params.length != 0) {
//            return;
            throw BindingSupportImpl.getInstance().internal(
                    "The query does not accept parameters, " +
                    "but valid params was passed for query excecution");
        }
        Object value = null;
        try {
            for (; p != null; p = p.next) {
                                ...
                                for (Iterator iterator = col.iterator(); iterator.hasNext();) {
                                    ...
                                    if (o instanceof OID) {
                                        ...
                                        throw BindingSupportImpl.getInstance().
                                                internal("Unexpected param type. Expexted collection of oids.");
                                    }
                                }
                            } else {
    ...
    public final static class Param implements Serializable {
         ...
         * parameters for the query or the index in the array of parameter
         * values used to exec the query (EJBQL). 
         */
         ...
         * @see #requiresUpdate
         */
        public boolean update(SqlBuffer q, boolean newParamIsNull) {
            boolean ans = false;
