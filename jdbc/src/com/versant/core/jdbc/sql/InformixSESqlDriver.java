...
/**
 * A driver for Informix SE using the ifxjdbc JDBC driver.
 */
public class InformixSESqlDriver extends SqlDriver {

    private NotSupportedConverter.Factory notSupportedConverterFactory =
            new NotSupportedConverter.Factory();
    private InformixSETrimStringConverter.Factory trimStringConverterFactory =
            new InformixSETrimStringConverter.Factory();

    public InformixSESqlDriver() {
    }
        ...
        switch (jdbcType) {
            ...
            case Types.CHAR:
                return new JdbcTypeMapping("CHAR",
                        116, 0, JdbcTypeMapping.TRUE, JdbcTypeMapping.TRUE,
                        null);
            case Types.VARCHAR:
                return new JdbcTypeMapping("CHAR",
                        116, 0, JdbcTypeMapping.TRUE, JdbcTypeMapping.TRUE,
                        trimStringConverterFactory);
            case Types.CLOB:
            case Types.LONGVARCHAR:
                return new JdbcTypeMapping("CHAR",
                        32000, 0, JdbcTypeMapping.TRUE, JdbcTypeMapping.FALSE,
                        trimStringConverterFactory);
    ...
    public HashMap getJavaTypeMappings() {
        ...
        // extended types
        add(ans,
                new JdbcJavaTypeMapping(File.class, Types.VARCHAR, -1, 0,
                        JdbcJavaTypeMapping.NOT_SET,
                        new FileConverterTrim.Factory(), false));
        add(ans,
                new JdbcJavaTypeMapping(URL.class, Types.VARCHAR, -1, 0,
                        JdbcJavaTypeMapping.NOT_SET,
                        new URLConverterTrim.Factory(), false));
    ...
    public String getConnectionValidateSQL() {
        return "select 42 from informix.systables where tabid = 1";
    }
        ...
        if (!params.checkColumnsOnly()) {
            ...
            if (params.isCheckPK()) {
                ...
                for (Iterator iterator = mainTableNames.iterator();
                    ...
                    while (rsPKs.next()) {
                        pkCount++;
                        pkMap.put(rsPKs.getString("COLUMN_NAME").trim(), null);
    ...
    protected void addSequenceColumn(JdbcTable t, JdbcColumn c, CharBuf s,
        ...
        s.append(comment("update the minimum id."));
        s.append("\n");
        s.append("UPDATE ");
        s.append(minTempTableName);
        s.append("\n   SET min_id = ");
